-- Migration: Remove Sobreaviso do fluxo de presenca (escala_diaria)
-- Data: 2026-08-07
--
-- REGRA DE NEGOCIO
--   Sobreaviso NAO marca presenca. O ciclo dele e proprio e vive inteiramente em
--   logs_sobreaviso: acionamento -> aceite (magic link enviado por WhatsApp/e-mail/SMS)
--   -> chegada ao local (GPS ou validacao manual). Nada disso entra na folha de ponto,
--   que le apenas as categorias Regular e Extra.
--
-- PROBLEMA
--   fn_confirmar_presenca incluia 'Sobreaviso' na lista de turnos do dia, o que causava:
--     a) fusao do sobreaviso no bloco de trabalho (corrigido em 20260807000000);
--     b) batidas de terminal caindo na linha de Sobreaviso de escala_diaria.
--   fn_confirmar_presenca_manual gravava presenca em escala_diaria tambem para Sobreaviso,
--   alem de registrar em logs_sobreaviso.
--
-- EVIDENCIA (producao)
--   Dos 522 acionamentos de sobreaviso registrados, NENHUM usou o terminal:
--     Manual 514 | GPS 8 | Terminal 0
--   O caminho de terminal para sobreaviso era codigo morto que so gerava efeito colateral.
--
-- CORRECAO
--   1. fn_confirmar_presenca deixa de considerar 'Sobreaviso' ao montar os blocos do dia
--      (lacos de ontem e de hoje). As checagens de ACESSO do coordenador continuam
--      aceitando Sobreaviso, para nao bloquear o terminal de quem so tem sobreaviso no dia.
--   2. fn_confirmar_presenca_manual, para p_categoria = 'Sobreaviso', deixa de escrever em
--      escala_diaria e passa direto para o registro em logs_sobreaviso. A validacao manual
--      de chegada (514 registros em producao) continua funcionando normalmente.
--
--   A barreira definitiva no banco vem em 20260807030000 (CHECK constraint).
--
-- ATENCAO AO RECRIAR ESTAS FUNCOES NO FUTURO
--   Ver .agents/AGENTS.md e CLAUDE.md.
--
-- Baseada em: 20260807000000_isolate_sobreaviso_from_work_block.sql
-- (corpo preservado byte a byte, exceto os 3 pontos abaixo)
