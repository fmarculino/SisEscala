-- Migration: Intervalo flexivel por servidor dentro de unidade rigida
-- Data: 2026-08-07
--
-- REGRA
--   Servidor com servidores.intervalo_flexivel = true pode gozar o intervalo em qualquer
--   horario, desde que cumpra a carga horaria liquida do dia. Os campos
--   intervalo_inicio_personalizado / intervalo_fim_personalizado (ou os padroes da jornada)
--   deixam de ser horarios obrigatorios e passam a definir apenas a DURACAO prevista.
--
--   saida_esperada = fim previsto da jornada + (intervalo real - intervalo previsto)
--
--     jornada 08h-18h, previsto 2h:
--       sai 11h volta 13h (2h)      -> saida 18h
--       sai 14h volta 17h (3h)      -> saida 19h   (+1h de excedente)
--       sai 12h volta 12h30 (30min) -> saida 16h30 (-1h30, carga liquida cumprida)
--       nao marcou intervalo        -> saida 18h, sinalizado como intervalo nao usufruido
--
-- COMPORTAMENTO NO TERMINAL (apenas quando intervalo_flexivel = true)
--   - Saida para intervalo: aceita em qualquer momento apos a entrada, desde que ANTES de
--     abrir a janela de saida final. Isso impede que a batida de saida do expediente seja
--     confundida com saida para intervalo (foi exatamente esse tipo de confusao que gerou
--     os defeitos corrigidos em 20260806000000).
--   - Retorno do intervalo: aceita em qualquer momento, mas somente se a saida para
--     intervalo ja tiver sido registrada.
--   - Saida final: janela normal de tolerancia, aplicada sobre o horario AJUSTADO.
--
--   Servidores com intervalo_flexivel = false seguem exatamente o comportamento anterior.
--
-- ATENCAO AO RECRIAR ESTA FUNCAO NO FUTURO
--   Preserve as chamadas a fn_ajuste_intervalo_flexivel nas janelas de saida e os ramos
--   de flexibilidade dos passos 2 e 3. Ver .agents/AGENTS.md e CLAUDE.md.
--
-- Baseada em: 20260807020000_remove_sobreaviso_from_presence_flow.sql


-- Ajuste, em minutos, a aplicar sobre a hora de saida prevista.
-- Positivo quando o servidor gozou mais intervalo que o previsto; negativo quando gozou menos.
-- Zero quando o servidor nao e flexivel ou nao registrou o intervalo completo.
CREATE OR REPLACE FUNCTION public.fn_ajuste_intervalo_flexivel(
    p_flexivel boolean,
    p_intervalo_saida timestamp with time zone,
    p_intervalo_retorno timestamp with time zone,
    p_intervalo_previsto_min integer
)
RETURNS integer
LANGUAGE sql
IMMUTABLE
...
AS $fn$
    SELECT CASE
        WHEN NOT COALESCE(p_flexivel, false) THEN 0
        WHEN p_intervalo_saida IS NULL OR p_intervalo_retorno IS NULL THEN 0
        ELSE (EXTRACT(EPOCH FROM (p_intervalo_retorno - p_intervalo_saida)) / 60)::integer
             - COALESCE(p_intervalo_previsto_min, 0)
    END;
$fn$;

COMMENT ON FUNCTION public.fn_ajuste_intervalo_flexivel(boolean, timestamp with time zone, timestamp with time zone, integer) IS
    'Minutos a somar a hora de saida prevista quando o servidor tem intervalo flexivel. Excedente adia, deficit antecipa.';
