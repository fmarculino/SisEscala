-- Migration: Support 4-step interval check-ins and afternoon check-in via terminal
-- Description: Updates fn_confirmar_presenca to calculate and evaluate all 4 intrajornada windows (Entrada 1º Período, Saída Intervalo, Retorno Intervalo / Entrada 2º Período, Saída Final). This allows servants to register interval exits/returns and check in for the 2nd period at ~13:00 even if they missed the morning period.
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            IF v_shifts_count = 1 THEN
                v_s1_id := r.escala_diaria_id; v_s1_inicio := v_start_min; v_s1_fim := v_end_min; v_s1_entrada := r.presenca_entrada_em; v_s1_int_saida := r.presenca_intervalo_saida_em; v_s1_int_retorno := r.presenca_intervalo_retorno_em; v_s1_saida := r.presenca_saida_em; v_s1_cat := r.categoria;
            ELSIF v_shifts_count = 2 THEN
                v_s2_id := r.escala_diaria_id; v_s2_inicio := v_start_min; v_s2_fim := v_end_min; v_s2_entrada := r.presenca_entrada_em; v_s2_int_saida := r.presenca_intervalo_saida_em; v_s2_int_retorno := r.presenca_intervalo_retorno_em; v_s2_saida := r.presenca_saida_em; v_s2_cat := r.categoria;
            ELSIF v_shifts_count = 3 THEN
                v_s3_id := r.escala_diaria_id; v_s3_inicio := v_start_min; v_s3_fim := v_end_min; v_s3_entrada := r.presenca_entrada_em; v_s3_int_saida := r.presenca_intervalo_saida_em; v_s3_int_retorno := r.presenca_intervalo_retorno_em; v_s3_saida := r.presenca_saida_em; v_s3_cat := r.categoria;
            END IF;
        END;
    END LOOP;
    ...
    LOOP
        ...
        BEGIN
            ...
            -- Calculate interval start and return target minutes
            IF r.permite_marca_intervalo AND (v_end_min - v_start_min) > 240 AND COALESCE(r.intervalo_minutos, 0) > 0 THEN
                v_permite_int := true;
                ...
                v_int_ini_min := CASE
                    ...
                    ELSE
                        v_start_min + 240 -- Fallback: 4 horas após a entrada
                END;
