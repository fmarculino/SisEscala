-- Migration: Restaura o guard de intervalo intrajornada para jornadas curtas
-- Data: 2026-08-06
--
-- PROBLEMA
--   A migration 20260804080000 recriou fn_confirmar_presenca e, ao faze-lo, OMITIU o
--   guard de duracao que existia em 20260804070000 (linha 691):
--
--       IF r.permite_marca_intervalo AND (v_end_min - v_start_min) > 240
--          AND COALESCE(r.intervalo_minutos, 0) > 0 THEN
--
--   Sem esse guard, servidores com jornada de 4h e 6h passaram a ter o fluxo de 4 batidas
--   (entrada / saida intervalo / retorno intervalo / saida), fazendo com que a saida real
--   fosse gravada como "saida para intervalo".
--
--   fn_confirmar_presenca_manual nunca teve o guard: aceita p_tipo 'intervalo_saida' e
--   'intervalo_retorno' para qualquer jornada. Como fn_confirmar_presenca_manual_bulk
--   apenas delega para ela, a validacao em massa herdava o mesmo defeito.
--
-- CORRECAO
--   1. Cria public.fn_jornada_tem_intervalo/2 como fonte unica da regra.
--   2. Restaura o guard nos dois lacos de fn_confirmar_presenca (blocos de ontem e de hoje),
--      agora com limite de 360 min (CLT Art. 71: intervalo apenas acima de 6h).
--   3. Adiciona guard em fn_confirmar_presenca_manual, cobrindo tambem o caminho _bulk.
--
-- REGRA (CLT Art. 71)
--   Intervalo intrajornada e obrigatorio apenas para jornadas SUPERIORES a 6h.
--   Jornadas de 4h e 6h passam a ter apenas entrada -> saida.
--
-- ATENCAO AO RECRIAR ESTAS FUNCOES NO FUTURO
--   O guard ja foi perdido uma vez em CREATE OR REPLACE. Ver .agents/AGENTS.md.
--
-- Baseada em: 20260804080000_fix_shift_n_18h_jornada_start.sql (corpo preservado byte a byte,
-- exceto os tres pontos de guard abaixo).
