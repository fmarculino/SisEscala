-- Migration: Add dynamic plantao alinhamento to contiguous regular shifts

-- 1. Helper function to fetch regular shift hours
CREATE OR REPLACE FUNCTION public.fn_obter_horario_regular_dia(
...
BEGIN
    ...
    JOIN public.jornadas j ON j.id = public.obter_jornada_servidor_data(em.servidor_id, MAKE_DATE(em.ano, em.mes, ed.dia), em.jornada_id)
    WHERE em.id = p_escala_mensal_id
      AND ed.dia = p_dia
      AND ed.categoria = 'Regular'
    LIMIT 1;

    IF v_jornada_nome IS NULL THEN
        RETURN NULL;
    END IF;

    v_start_hour := substring(v_jornada_nome from '^([0-9]+)')::integer;
    v_end_hour := substring(v_jornada_nome from '(?:ÀS|AS|as|às)\s*([0-9]+)')::integer;
...
-- 2. Update fn_salvar_saida_bloco to dynamically calculate start hour
...
-- 3. Drop function to avoid overloading problems
DROP FUNCTION IF EXISTS public.fn_confirmar_presenca(text, text, uuid, timestamp with time zone);

-- 4. Recreate main confirmation function with alinhamento dinâmico
                    ...
                    IF v_b_fim > 1440 AND v_b_entradas[1] IS NOT NULL AND v_b_saidas[v_b_total_count] IS NULL THEN
                        ...
                        BEGIN
                            ...
                            IF v_pf_existe AND v_pf_inicio_minutos IS NOT NULL THEN
                                ...
                                -- But if it's late entry or another setup, we can also evaluate. Normally, crossing midnight shifts
                                -- aren't affected by next day's early release. Still, let's keep it safe.
                            END IF;
...
-- 5. Drop old fn_confirmar_presenca_manual versions
DROP FUNCTION IF EXISTS public.fn_confirmar_presenca_manual(uuid, integer, text, text, uuid);

-- 6. Recreate fn_confirmar_presenca_manual with alinhamento dinâmico
        ...
        COALESCE(
            ...
            CASE WHEN ed.categoria = 'Plantão' THEN
                COALESCE(
                    CASE 
                      WHEN (dt.codigo LIKE 'T%' OR dt.slots[1] = 'T') AND 
                           (public.fn_obter_horario_regular_dia(p_escala_mensal_id, p_dia)->>'end_hour')::integer BETWEEN 11 AND 15
                      THEN (public.fn_obter_horario_regular_dia(p_escala_mensal_id, p_dia)->>'end_hour')::integer

                      WHEN (dt.codigo LIKE 'N%' OR dt.slots[1] = 'N') AND 
                           (public.fn_obter_horario_regular_dia(p_escala_mensal_id, p_dia)->>'end_hour')::integer BETWEEN 17 AND 20
                      THEN (public.fn_obter_horario_regular_dia(p_escala_mensal_id, p_dia)->>'end_hour')::integer

                      WHEN (dt.codigo LIKE 'M%' OR dt.slots[1] = 'M') AND 
                           (public.fn_obter_horario_regular_dia(p_escala_mensal_id, p_dia)->>'start_hour')::integer BETWEEN 12 AND 15
                      THEN (public.fn_obter_horario_regular_dia(p_escala_mensal_id, p_dia)->>'start_hour')::integer - dt.horas_computadas::integer
