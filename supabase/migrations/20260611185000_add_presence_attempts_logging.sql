-- Migration: Add presence attempts logging
-- Description: Creates logs_tentativas_presenca table and helper logging function, and updates fn_confirmar_presenca to log denied attempts.

CREATE TABLE IF NOT EXISTS public.logs_tentativas_presenca (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    servidor_id UUID REFERENCES public.servidores(id) ON DELETE SET NULL,
    matricula_digitada TEXT,
    nome_servidor_detectado TEXT,
    data_hora_tentativa TIMESTAMP WITH TIME ZONE DEFAULT now(),
...
CREATE POLICY "Allow super_admin read logs" ON public.logs_tentativas_presenca
...
-- Create policy for inserts from authenticated users
...
-- Recreate main confirmation function
                        ...
                        SELECT 
                            COALESCE(
                                (
                                    ...
                                    JOIN public.jornadas j2 ON em2.jornada_id = j2.id
                                    WHERE em2.id = em.id
                                      AND ed2.dia = ed.dia
                    ...
                    WHERE em.servidor_id = v_servidor_id
                      ...
                      AND (
                          (v_closest_action = 'checkin' AND (extract(hour from COALESCE(
                              ...
                              (
                                  SELECT COALESCE(
                                      (SELECT substring(j2.nome from '(?:ÀS|AS|as|às)\s*([0-9]+)')::integer FROM public.escala_diaria ed2 JOIN public.escala_mensal em2 ON ed2.escala_mensal_id = em2.id JOIN public.jornadas j2 ON em2.jornada_id = j2.id WHERE em2.id = em.id AND ed2.dia = ed.dia AND ed2.categoria = 'Regular' LIMIT 1),
