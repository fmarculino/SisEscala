-- Migration: Fix Checkin vs Checkout Assignment using Shift Midpoint Temporal Logic
-- Description: Updates fn_confirmar_presenca to compare current timestamp against the shift block midpoint (v_b_midpoint = (v_b_inicio + v_b_fim)/2) when entry and exit are both unrecorded. If current time is past the shift midpoint, Step 1 (Checkin) is bypassed so the punch is correctly recorded as Checkout (presenca_saida_em). Also fixes existing anomalous rows in escala_diaria where an exit punch was incorrectly saved in presenca_entrada_em. Maintains dynamic extra hour alignment per AGENTS.md rule.

-- 1. RECREATE fn_confirmar_presenca WITH SHIFT MIDPOINT TEMPORAL LOGIC
...
BEGIN
        ...
        ELSIF v_shifts_count >= 3 THEN
            ...
            ELSE
                ...
                IF v_s3_inicio <= v_s2_fim THEN
                    ...
                    v_b2_int_ini := COALESCE(v_s2_int_ini_min, v_s3_int_ini_min); v_b2_int_fim := COALESCE(v_s2_int_fim_min, v_s3_int_fim_min); v_b2_permite_int := COALESCE(v_s1_permite_int, v_s2_permite_int, false);
                ELSE
                    v_blocks_count := 3;
                    v_b2_inicio := v_s2_inicio; v_b2_fim := v_s2_fim; v_b2_ids := ARRAY[v_s2_id]; v_b2_entradas := ARRAY[v_s2_entrada]; v_b2_int_saidas := ARRAY[v_s2_int_saida]; v_b2_int_retornos := ARRAY[v_s2_int_retorno]; v_b2_saidas := ARRAY[v_s2_saida]; v_b2_cat := v_s2_cat; v_b2_int_ini := v_s2_int_ini_min; v_b2_int_fim := v_s2_int_fim_min; v_b2_permite_int := v_s2_permite_int;
    ...
    FOR r IN 
        SELECT 
            ...
            COALESCE(
                ...
                ELSE NULL END,
                -- Se for Extra, busca o término do turno Regular ou Plantão do mesmo dia com tratamento de meia-noite (MANTIDO CONFORME AGENTS.MD)
                (
                    SELECT 
                        COALESCE(
                            (
    ...
    ELSIF v_shifts_count >= 3 THEN
        ...
        ELSE
            ...
            IF v_s3_inicio <= v_s2_fim THEN
                v_blocks_count := 2;
                v_b2_inicio := v_s2_inicio; v_b2_fim := GREATEST(v_s2_fim, v_s3_fim); v_b2_ids := ARRAY[v_s2_id, v_s3_id]; v_b2_entradas := ARRAY[v_s2_entrada, v_s3_entrada]; v_b2_int_saidas := ARRAY[v_s2_int_saida, v_s2_int_saida]; v_b2_int_retornos := ARRAY[v_s2_int_retorno, v_s3_int_retorno]; v_b2_saidas := ARRAY[v_s2_saida, v_s3_saida];
                v_b2_cat := CASE WHEN v_s2_cat IN ('Regular', 'Plantão') THEN v_s2_cat ELSE v_s3_cat END;
                v_b2_int_ini := COALESCE(v_s2_int_ini_min, v_s3_int_ini_min); v_b2_int_fim := COALESCE(v_s2_int_fim_min, v_s3_int_fim_min); v_b2_permite_int := COALESCE(v_s1_permite_int, v_s2_permite_int, false);
            ELSE
                v_blocks_count := 3;
    ...
    BEGIN
        FOR idx IN 1..v_blocks_count LOOP
            ...
            v_b_total_count := array_length(v_b_ids, 1);
            v_b_midpoint := (v_b_inicio + v_b_fim) / 2;

            -- Step 1: Checkin (first shift entry is NULL, tap is BEFORE shift midpoint or exit is present, and tap matches start window)
            IF v_b_entradas[1] IS NULL THEN
                v_closest_inicio_formatted := lpad((v_b_inicio / 60)::text, 2, '0') || ':' || lpad((v_b_inicio % 60)::text, 2, '0');
                
                IF (v_b_saidas[v_b_total_count] IS NOT NULL OR v_momento_atual_minutos < v_b_midpoint) AND (
                   v_ignora_janela OR (
        ...
        IF v_matched_action IS NULL THEN
            FOR idx IN 1..v_blocks_count LOOP
                ...
                IF v_b_entradas[1] IS NULL AND (v_b_saidas[v_b_total_count] IS NOT NULL OR v_momento_atual_minutos < v_b_midpoint) THEN
...
-- 2. DATA CORRECTION FOR EXISTING ANOMALOUS PRESENCE RECORDS IN escala_diaria
-- Fix: rows where presenca_entrada_em was erroneously recorded at shift exit time
-- (entry recorded after shift midpoint, with no exit recorded)
-- Move the value from presenca_entrada_em -> presenca_saida_em and clear presenca_entrada_em
UPDATE public.escala_diaria
SET presenca_saida_em = anomaly.presenca_entrada_em,
    presenca_entrada_em = NULL,
    presenca_saida_manual = anomaly.presenca_entrada_manual,
    presenca_entrada_manual = false
FROM (
    SELECT 
        ed.id,
        ed.presenca_entrada_em,
        ed.presenca_entrada_manual
    FROM public.escala_diaria ed
    JOIN public.escala_mensal em ON ed.escala_mensal_id = em.id
    LEFT JOIN public.jornadas j ON j.id = public.obter_jornada_servidor_data(em.servidor_id, MAKE_DATE(em.ano, em.mes, ed.dia), em.jornada_id)
    WHERE ed.presenca_entrada_em IS NOT NULL
      ...
      AND (
          -- Day shift: entry recorded after shift midpoint (e.g. 07H AS 13H, midpoint 10:00)
          (
              COALESCE(substring(j.nome from '^([0-9]+)')::integer, 7)
                < COALESCE(substring(j.nome from '(?:ÀS|AS|as|às)\s*([0-9]+)')::integer, 13)
              AND extract(hour from (ed.presenca_entrada_em AT TIME ZONE COALESCE(
                      (SELECT valor#>>'{}' FROM public.configuracoes_globais WHERE chave = 'timezone'),
                      'America/Sao_Paulo'
                  )))::integer >=
                  ( (COALESCE(substring(j.nome from '^([0-9]+)')::integer, 7)
                    + COALESCE(substring(j.nome from '(?:ÀS|AS|as|às)\s*([0-9]+)')::integer, 13))
          ...
          OR
          -- Overnight shift (e.g. 19H AS 07H): entry local hour is early morning (05:00-11:00)
          (
              COALESCE(substring(j.nome from '^([0-9]+)')::integer, 19)
                > COALESCE(substring(j.nome from '(?:ÀS|AS|as|às)\s*([0-9]+)')::integer, 7)
              AND extract(hour from (ed.presenca_entrada_em AT TIME ZONE COALESCE(
                      (SELECT valor#>>'{}' FROM public.configuracoes_globais WHERE chave = 'timezone'),
                      'America/Sao_Paulo'
...
WHERE public.escala_diaria.id = anomaly.id;
