-- Migration: Fix Extra Shifts Midnight Alignment and Presence Validation
-- Description: Restores dynamic start_hour calculation for Extra category shifts (aligning Extra start to the end of Regular or Plantão shifts on the same day, including midnight crossing for Plantão N) in both fn_confirmar_presenca and fn_confirmar_presenca_manual.

-- 1. RECREATE fn_confirmar_presenca WITH RESTORED EXTRA SHIFT ALIGNMENT
...
-- 2. RECREATE fn_confirmar_presenca_manual WITH RESTORED EXTRA SHIFT ALIGNMENT
...
BEGIN
    ...
    -- Calculate scheduled start and end timestamps in local time
    ...
    IF p_categoria = 'Sobreaviso' THEN
        v_motivo_log := 'Validação Manual (' || p_categoria || ' - ' || p_tipo || ') por gestor.';
        IF p_justificativa IS NOT NULL AND length(trim(p_justificativa)) > 0 THEN
