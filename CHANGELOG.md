...
### Fixed (correção dentro da própria versão)
- 🚨 **`fn_avisos_ponto_pendentes` ficou quebrada em produção entre as duas migrations** — `20260830160000` conserta. `RETURNS TABLE (… canal, destino, tentativas …)` declara parâmetros de **saída** com esses nomes, e o corpo faz `UPDATE … SET tentativas = …, canal = …, destino = …` sobre colunas homônimas. **O alvo de um `SET` não pode ser qualificado** (`SET f.canal` é erro de sintaxe), então não há como desambiguar escrevendo melhor. Erro `42702`.
  - ⚠️ **Nem `tsc`, nem `lint`, nem `build` pegam isso:** plpgsql só resolve nome na **execução** (armadilha 1). A função foi criada sem reclamar — e como a migration anterior tinha feito `DROP` da assinatura antiga (para evitar `PGRST203`, armadilha 41), **a única versão existente passou a ser a quebrada** e o despacho parou por completo, em silêncio.
  - Corrigido com `#variable_conflict use_column`. ⚠️ **A verificação da migration passou a EXECUTAR a função**, com argumentos sem efeito colateral (`(1, 0)`, que só alcança item de e-mail): conferir que ela **existe** não serviria de nada, porque ela existia e estava quebrada.
  - Quem pegou foi a sonda de conferência por fora (`scratchpad/an_confere_canal_aviso.mjs`), não a migration. **Armadilha 42** no CLAUDE.md.
...
### Added
- **Excluir setor que TEM vínculo: transferir tudo para outro setor e então excluir** (`fn_fundir_setor`, `20260829110000`). A exclusão criada em 2.21.0 só alcança setor **sem nenhum vínculo** — medido em produção nos 646 setores, isso eram **200**. E dos **16 já inativos** (os que alguém quer justamente tirar do cadastro), **7 estavam presos para sempre**: a recusa listava os vínculos e não oferecia ação nenhuma a partir dali.
  - **Cascata foi considerada e descartada, e não deve voltar.** As três maiores tabelas presas ao setor são `marcacoes_ponto` (26.834 linhas), `escala_mensal` (1.658) e `servidores` (1.396). Apagar em cascata é **destruir registro de ponto de servidor público** — prova legal (Portaria 671/2021) — para resolver problema de cadastro. A fusão move o dono e não perde nada.
  - **A varredura é dinâmica sobre `pg_constraint`**, mesma escolha de `fn_dependencias_setor`: as tabelas base não estão versionadas (armadilha 2), então lista escrita à mão nasceria incompleta e envelheceria a cada tabela nova. Hoje são **14 colunas de FK com uso real**.
  - **A fusão RECUSA em bloco** (`fn_impedimentos_fusao_setor`, consultada pela tela a cada troca do `<select>`): destino em outra unidade (mover servidor e escala de unidade é **transferência**, tem tela e regra próprias), destino que é **subsetor** da origem (viraria pai de si mesmo, e ciclo em `parent_id` trava a montagem de árvore de toda tela de setor), o mesmo servidor com escala nos **dois** setores na mesma competência (unique de `escala_mensal`; juntar contaria as horas duas vezes na folha) e qualquer outra colisão de unicidade. As duas exceções em que a duplicata é descartada são `profile_setores` e `dispositivos_rep_setores` — ali a linha não tem dado próprio, ela **é** o par.
  - 🚨 **Abre a primeira exceção no trigger de imutabilidade da marcação, e ela é estreita de propósito.** Sem isso, os **107 setores** que já tiveram batida seriam infundíveis (a FK barra o `DELETE`). O ramo novo exige o GUC `sisescala.fundir_setor` (local à transação, como o `sisescala.reparse_afd`) **e** que o `UPDATE` altere exclusivamente `setor_id` — a comparação é `to_jsonb(NEW) - 'setor_id' = to_jsonb(OLD) - 'setor_id'`, **estrutural**, não uma lista de campos que envelhece: horário, servidor, origem, dispositivo e NSR continuam impossíveis de alterar aí mesmo depois de a tabela ganhar coluna nova. O fato registrado não muda; muda o rótulo de contexto "em que setor isso aconteceu". A operação vira linha em `logs_sistema` (`setor_fundido`), com o de → para e a contagem por tabela.
  - Conferido antes: `trg_enfileirar_aviso_ponto` e `trg_reconciliar_apos_marcacao` são **AFTER INSERT**, então a fusão não dispara aviso nem reconciliação para as 26.834 marcações.
  - Na tela, o modal consulta as dependências **ao abrir** — antes o bloqueio só aparecia na recusa. Com vínculo, mostra a lista traduzida ("1 servidores lotados aqui", "22 marcações de ponto"), o `<select>` de destino (ativos, mesma unidade, **sem os subsetores do próprio setor**) e o botão vira "Transferir e excluir".
- **Seleção de setores em árvore, com marcação em cascata** (`src/components/setores/SeletorSetoresArvore.tsx`), no modal do Dispositivo REP. Clicar num setor pai marca (ou desmarca) **todos os subsetores dele**; o pai mostra estado parcial e o contador `marcados/total`. Tem expandir/recolher por nó, "Marcar todos" / "Limpar" e filtro por nome. O HMM tem **196 setores em 40 raízes e 3 níveis** — a lista plana anterior mostrava a hierarquia só como recuo no texto e obrigava a marcar dezenas de caixas uma a uma.
- **Expandir e recolher na tela `/setores`**: "Expandir tudo" / "Recolher tudo" por unidade, e o **card da unidade inteiro passou a ser recolhível** (o cabeçalho virou o botão), com "Recolher todas as unidades" / "Expandir todas" na barra e o contador "N de M unidades abertas" — são 33 unidades na página.
- **IP da máquina do coletor** (`dispositivos_rep.coletor_ip`, `20260829120000`; coletor **v0.13.0**). A tela já mostrava o hostname (`máquina: HMM-CCE-NI`), e ele não leva ninguém até o computador: não há DNS interno cobrindo as unidades. O que existia era `endereco_ip` (o **relógio**) e `ultimo_ip_origem`, que é o **IP público** — medido em 29/08, as **cinco máquinas do HMI aparecem todas com `45.173.175.9`**.
  - ⚠️ O IP é descoberto por `net.Dial("udp", <relógio>)`, que **não envia pacote nenhum** — só faz o sistema escolher a rota e revelar a interface. Numa máquina com várias placas, "a primeira IPv4 não-loopback" acerta por sorte: na máquina de dev a varredura devolvia primeiro um `169.254.87.133` (link-local de adaptador virtual) enquanto o Dial devolveu o `10.110.2.111` correto. O fallback ficou, pulando loopback **e** link-local.
- **"Onde estão as horas" virou link para a grade** em `/relatorios/carga-consolidada` (`20260829130000`). Quem aparece nessa lista está acima do teto — alguém precisa **abrir** aquela escala para reduzir, e o caminho era decorar unidade e setor e procurar em Escalas. `fn_carga_mensal_servidor` já devolvia `unidade_id` e `setor_id`; era a CTE e o `jsonb_build_object` do relatório que não os repassavam.

### Fixed
- **Unidade e setor INATIVOS continuavam sendo oferecidos para escolha.** O `<select>` de unidade do modal do Dispositivo REP listava as 33 unidades sempre, a inativa junto — escolher uma cria vínculo novo com cadastro que alguém já aposentou, sem a tela dizer nada. Fonte única em `src/utils/opcoesAtivas.ts`.
  - **A regra tem dois lados, e os dois importam**: inativo **some de onde se escolhe** onde algo vai ficar (relógio, terminal, escopo de usuário, transferência, lotação) e **continua nos filtros** de listagem e relatório, rotulado `(inativo)` — escala, folha e ponto registrados naquele setor não deixaram de existir, e sem a opção no filtro ficam inalcançáveis pela tela.
  - ⚠️ **Filtrar na CONSULTA é o erro oposto**, e era o que o modal do REP já fazia com setores: o relógio que atende um setor depois desativado continua atendendo, com a caixa **invisível** na única tela que gerencia aquilo. Por isso a lista chega completa com a flag `ativo` e quem esconde é a tela, que conhece a seleção atual.
  - ⚠️ **O já selecionado nunca some**, mesmo inativo: tirá-lo faz o `<select>` exibir vazio para um registro que aponta para ele, e o próximo "Salvar" grava a troca que ninguém pediu.
  - Medido: **1 unidade inativa** (de 33) e **16 setores inativos** (de 646); nenhum setor ativo pendurado em unidade inativa ou em pai inativo.
- **O histórico de sobreaviso oferecia acionar um plantão que já passou.** O modal "Histórico de Acionamentos" trazia "Novo Acionamento neste Dia" **sempre habilitado**, inclusive num plantão de semanas atrás. Era decisão consciente da Fase 8 (a heurística de janela do frontend divergia da do banco, então tirou-se a heurística e deixou-se a RPC recusar "com o horário exato"); a metade que não se sustentou é que, num dia passado, o botão **convida** a fazer algo impossível — quem clica só descobre depois de escrever o motivo.
  - ✅ **Nada era gravado**: `fn_acionar_sobreaviso` já recusa fora da janela. O defeito era de oferta.
  - **A correção não reintroduz heurística**: o modal consulta `fn_janela_sobreaviso_dia` — a **mesma** função que autoriza a gravação — e desabilita o acionamento **com** a janela exata escrita ao lado ("Este plantão valia de 02/08/2026 07:00 até 03/08/2026 07:00"), apontando o caminho que continua valendo (**Validar Este Chamado (Manual)**). Botão cinza sem explicação continua proibido.
  - ⚠️ **"Reenviar Notificação / Link" leva a mesma trava**: ele **reabre o modal de acionamento** com o motivo preenchido, gerando chamado novo — não é reenvio passivo.
- **A busca da tela `/setores` mostrava o subsetor solto na raiz da unidade.** O filtro derrubava o pai que não casava com o termo, e o laço da árvore promove a raiz todo setor cujo pai não está na lista — a busca respondia "onde está" tirando justamente a resposta. Os ancestrais de quem casou passam a entrar na lista, e ramo e unidade ficam abertos enquanto houver busca.
- **"Toda a unidade" no modal do relógio virou um MODO, não `setorIds.length === 0`.** Derivado da lista vazia, o botão "Limpar" da árvore trocava o significado do formulário sozinho: desmarcar o último setor voltava para "toda a unidade" e a árvore sumia da tela. O banco continua guardando as duas coisas igual (nenhuma linha em `dispositivos_rep_setores`), mas salvar sem setor nenhum agora é **recusado** com mensagem.
- **`fn_dependencias_setor` estava aberta a `anon`**: a migration que a criou fez só `GRANT`, e `GRANT` nunca restringiu nada — toda função nova nasce executável por `PUBLIC` (armadilha 24). Fechada com `REVOKE ... FROM PUBLIC, anon`.

### Known
- ⚠️ **As três migrations desta versão não foram aplicadas em homologação** — a tentativa de validar lá foi barrada pelo ambiente, e o fluxo real deste projeto aplica direto em produção. `20260829110000` (fusão), `20260829120000` (`coletor_ip`) e `20260829130000` (link da carga) precisam ir junto com o deploy; até lá o app degrada de forma legível (o botão de transferir falha com a mensagem do banco, o IP não aparece, a linha do relatório não vira link).
- ⚠️ **A `20260829130000` foi reprovada na primeira aplicação** com `42703: column c.unidade_id does not exist` — a CTE `carga` projeta uma lista explícita de colunas, e acrescentar a chave no `jsonb` sem acrescentar lá não é erro que `tsc`, `lint` ou `build` peguem: SQL só resolve nome de coluna na execução (armadilha 1). O gerador passou a abortar se qualquer uma das duas âncoras não aparecer exatamente uma vez.
- ⚠️ **Publicar a v0.13.0 do coletor faz o parque se auto-atualizar** em até 24h + atraso sorteado (auto-update da v0.12.0). A mudança é de uma função só (o IP no heartbeat), mas o risco é o de sempre: um release ruim alcança as máquinas que ninguém alcança fisicamente.
- ⚠️ **O botão de raio da própria célula da grade ainda decide por heurística de prefixo de código** (`N` → 19h–07h, `T` → 13h–19h…) — exatamente o que a Fase 8 tirou do modal. Coincidiu nos casos medidos, mas é uma segunda conta para a mesma pergunta; alinhar exige carregar a janela do mês inteiro, não de um dia.
- ⚠️ Continuam abertas as pendências de 2.22.0: o ponto cego do motor de compliance entre setores, a `FOR ALL` de `historico_transferencias`, e as auditorias das 533 marcações sintéticas e dos 10 dias com origem `manual` em 08/2026.

## [2.22.0] - 2026-08-28

### Fixed
- 🚨 **O teto de 300h por servidor era conferido só dentro de UMA grade, e a pessoa pode estar em várias.** `configuracoes_globais.max_horas_escala_servidor` sempre foi um limite **da pessoa no mês**, mas a única conta que o defendia era `calculateTotals(servidorId)`, que soma o `gridData` daquele setor. Servidor escalado em dois lugares tinha duas contas dentro do teto e uma soma fora dele — com as duas telas mostrando um número válido.
  - **Caso que revelou:** JEANE CONCEICAO SILVA, 09/2026, HMI — `SHL \ ACOLHIMENTO` com **289h** e `SHL \ LAVANDERIA` com **120h**. **409h** para a mesma pessoa no mesmo mês.
  - ⚠️ **E `handleCellChange` era o ÚNICO chamador da checagem em todo o repositório** — ou seja, só a digitação célula a célula. **Aplicar Template**, **Gerador Inteligente** e a gravação dos meses extras nunca consultaram o teto, nem dentro do próprio setor. **Não existia nada no banco**: nenhum trigger, nenhuma função, e `max_horas_escala_servidor` aparecia em **uma** migration, a que cria a chave. É a armadilha 14 e a 23 num terceiro eixo — lá o furo do template era afastamento e sobreposição entre setores, aqui era carga horária.
  - **Medido em produção:** 49 servidores em 2+ escalas em **09/2026**, contra 2 ou 3 em 06, 07 e 08 — o caso não era antigo, estava nascendo no planejamento de setembro. Três estouravam o teto **só somando** (JEANE 409h, EDIVONETE 314h, ERIKA SOUZA 302h), todos HMI e em Rascunho. 🚨 **A lista cresceu enquanto a correção era escrita**: no fim da mesma tarde eram **5**, com NANCI (326h) e MARIA KEDMA (314h) entrando sem que ninguém fosse avisado de nada.
  - **Banco** (`20260828120000`): `fn_carga_mensal_servidor` devolve uma linha por escala do servidor na competência, com unidade, **caminho completo do setor** e horas; `fn_teto_carga_servidor` devolve o teto efetivo (global + autorização); `fn_setor_caminho` é o espelho SQL de `buildSectorPathMap`. As duas primeiras recebem **lista** de servidores — uma chamada por linha da grade seriam dezenas de requisições por carregamento.
  - **Frontend**, com fonte única em `src/utils/limiteCargaMensal.ts`: os **quatro** caminhos de escrita passaram a conferir, mais a barreira do "Salvar Previsão", que **relê a carga do banco** (aba desatualizada é o caso que a checagem local não cobre). A coluna **TOTAL H/MÊS** ganhou as linhas **Outras** e **Mês**, com tooltip listando `UNIDADE / SETOR — Nh`, e fica vermelha acima do teto. Um **escudo vermelho** na linha do servidor abre a Autorização Extraordinária. E há aviso ao **adicionar** o servidor à grade — antes de lançar o mês dele, que é onde o aviso custa menos.
  - ⚠️ **A escala da própria grade é excluída da carga vinda do banco e substituída pelo total local**: o banco tem o que foi salvo, a grade tem o que está sendo lançado, e somar os dois contaria o mesmo turno duas vezes.
  - ⚠️ **`decomporPlantao` não foi replicada no SQL, e tentar isso seria o erro**: o total é exatamente `SUM(horas_computadas)` — as unidades PL existem para as colunas de pagamento, nunca para o total. Somar por faixa de duração ali reintroduziria o bug de 21/08/2026 (44 dos 53 códigos contando errado) dentro da própria trava.
  - ⚠️ **Não há trigger no banco, e é decisão registrada**: um trigger duro exigiria a autorização gravada **antes** do upsert em lote, invertendo a ordem do fluxo. Como consequência assumida, a barreira do `handleSave` é a **última** defesa e **recusa em caso de falha de rede** — ao contrário da de sobreposição entre setores, onde o trigger segura.
  - Portão: `node scratchpad/sim_limite_carga.js` (45 casos). Registrado como **armadilha 26** no `CLAUDE.md`; diário em [`docs/evolucao/2026-08-28-teto-de-horas-entre-escalas.md`](docs/evolucao/2026-08-28-teto-de-horas-entre-escalas.md).
- 🚨 **A policy que dizia "avaliação de transferência só super_admin" nunca restringiu nada.** Policies permissivas se somam com `OR`, e `20260818100000` criou na mesma tabela uma policy **`FOR ALL`** (ampliada para `ass_adm` em `20260818170000`). `FOR ALL` cobre `UPDATE`, e sem `WITH CHECK` próprio ele cai para o `USING` — então **`admin`, `coordenador`, `rh_unidade` e `ass_adm` podiam marcar um pedido como `aprovada` chamando o PostgREST direto**. O que os segurava era o `if` da server action. É a **armadilha 24 em outra forma**: a policy estrita existe, e a permissiva ao lado dela é que decide. `20260828100000` derruba a `FOR ALL` e escreve `SELECT`, `INSERT` e `UPDATE` separados — quem **solicita** não perde nada.
- **Filtro de setor que nunca filtrou.** Os dois `<select>` de `/servidores/pendencias` fazem `.filter(s => s.ativo !== false)` para não oferecer setor desativado, mas a página montava a lista sem repassar `ativo` — e `undefined !== false` é **sempre true**, então os **17 setores inativos** continuavam aparecendo. O comentário no componente descrevia um comportamento que o dado não sustentava.

### Added
- **Relatório "Carga Consolidada do Mês"** (`/relatorios/carga-consolidada`, `20260828130000`). A checagem dentro da grade resolve para quem está lançando, **e só ali**: a informação "esta pessoa tem 409h somando dois setores" só aparecia se alguém abrisse justamente uma das duas grades — quem confere o mês teria que abrir 96 setores para achar 3 pessoas.
  - Lista quem está em **2+ escalas com carga** ou **acima do teto**, com o total, o teto, a situação e a composição (`unidade / setor — Nh`). Listar todo mundo seriam 500 linhas por competência para achar 3, e o corte silencioso de 1000 do PostgREST ficaria a uma competência de distância.
  - ⚠️ **Sem filtro de unidade nem de setor, de propósito**: a pergunta é "quanto esta PESSOA tem no mês", e a resposta cruza setores por definição — filtrar devolveria a conta parcial que o relatório existe para corrigir.
  - O escopo é resolvido **por unidade, uma vez cada** (são ~1.600 escalas por competência contra algumas dezenas de unidades), e o guard de papel é **denylist** — allowlist esquece papel novo, como aconteceu com `rh` e `rh_unidade` em `fn_painel_sobreaviso_dia`.
- **RH avalia solicitação de transferência.** Aprovar/rejeitar deixou de ser exclusividade do Administrador Geral: **RH Geral avalia qualquer pedido; RH da Unidade avalia dentro das unidades dele**. Os demais papéis continuam apenas solicitando, e `updateServidor` não mudou — `super_admin` e `rh` transferem direto, todos os demais solicitam.
  - Fonte única em `src/utils/avaliacaoTransferencia.ts`, aplicada nos **três** lugares: tela (`podeAvaliar` resolvido no servidor, linha a linha), action (mensagem legível) e policy de `UPDATE` (o que o banco grava).
  - ⚠️ **`rh_unidade` aprova só com ORIGEM *e* DESTINO no escopo dele**: a policy de escrita de `servidores` só aceita `unidade_id ∈ profile_unidades` nesse braço, e o `WITH CHECK` roda sobre a linha nova — transferência para fora seria recusada lá de qualquer forma, com o sintoma mudo *"Nenhuma alteração foi gravada"*. O destino é conferido pelo **valor final**, o do pedido ou o que o avaliador acabou de escolher. E **`acesso_todas_unidades` não vale como bypass** para esse papel.
- **Caminho completo de setor em texto solto e em `<select>`** (`buildSectorPathMap` / `formatSectorPaths` em `src/utils/sectors.ts`). Nome de setor sozinho não identifica setor: "BLOCO A" existe embaixo de mais de um pai, e a saída que vinha sendo usada era **batizar o dicionário de "BLOCO A SHL"** — escrever a hierarquia dentro do nome, duplicando no cadastro o que o `parent_id` já sabe. O separador é **barra invertida** (`SHL \ BLOCO A`) de propósito: a tela já usa `" / "` entre unidade e setor, e repetir a barra normal apagaria essa fronteira.
  - ⚠️ **Não substitui `formatSectorsHierarchy`**: o recuo com `↳` serve para lista curta, onde o pai fica na linha de cima; o caminho serve para texto solto e `<select>` longo, onde o pai sai da tela ao rolar.
  - ⚠️ **O embed `setores(dicionario_setores(nome))` só traz a FOLHA** — por isso o caminho não sai de graça em consulta nenhuma. `buscarCaminhosDeSetor` busca a árvore inteira, **paginada** (são 645 setores, perto demais do teto de 1000) e **sem filtro por unidade**: bastaria um pai cadastrado em outra unidade para o caminho do filho ficar curto, parecendo certo.
  - Aplicado em `/servidores/pendencias`, `/escalas` e `/escalas/unidade/[id]` (cabeçalho e PDF). Ainda mostram só a folha, e são a fila natural: `/folha-ponto`, `/afastamentos`, `/ferias-licencas`, `/justificativas`, `/marcacoes`, `/servidores`, `/auditoria` e os relatórios.

### Changed
- **A Autorização Extraordinária passou a ser UMA por (servidor, mês, ano).** A chave era `(servidor, unidade, mês, ano)` — com o teto consolidado, duas unidades concederiam +100h cada e o teto efetivo viraria 500h sem que ninguém tivesse decidido isso; somar as autorizações apaga o teto, e pegar a maior faz o teto depender de qual unidade agiu primeiro. `unidade_id` fica como registro de **onde** a autorização foi dada. A tabela tinha **0 linhas** em produção — em um ano, ninguém exerceu o teto uma vez sequer —, o que tornou a troca de chave gratuita.
- **O modal de Autorização Extraordinária mostra a composição do mês** (esta escala + as outras, com unidade e caminho do setor), e a margem sugerida passou a cobrir o excesso do **mês inteiro** — era onde ela saía curta. Sem isso o administrador via "409h" numa grade que mostra 120h, sem como conferir nada.
- `fetchExcecoesEscala` perdeu o filtro por unidade: com a autorização valendo para a pessoa, filtrar faria uma grade ignorar a autorização concedida a partir do outro setor e recusar um lançamento já autorizado.
- `calculateTotals` ganhou um parâmetro `override` para **simular** um lançamento antes de escrevê-lo — sem ele, Aplicar Template e Gerador Inteligente teriam que reimplementar a fórmula de horas, inclusive o teto líquido da jornada.

### Known
- ⚠️ **Os servidores acima do teto precisam ser resolvidos, senão os setores deles não conseguem salvar.** Medido em 28/08 ao fim do dia: **8 setores do HMI em 09/2026** (JEANE 409h, NANCI 326h, EDIVONETE 314h, MARIA KEDMA 314h, ERIKA SOUZA 302h) e a **TI da SMS em 08/2026** (23 unidades de sobreaviso contra teto de 20). Para cada um: reduzir a escala ou conceder a Autorização Extraordinária pelo escudo vermelho na grade. Os dois casos do LACEM (07/2026) **não travam nada** — competência encerrada e escalas Fechadas, anteriores à regra. **A lista está viva**: rode `node scratchpad/an_limite_horas.mjs` na hora.
- ⚠️ **O motor de compliance tem o MESMO ponto cego e continua com ele.** `runComplianceCheck` recebe só o `gridData` local, então **interjornada e DSR não enxergam o outro setor** — a mesma pessoa pode ter `MT` num setor e `N` no outro em dias vizinhos sem nenhum dos dois alertas disparar. Mesmo defeito, eixo diferente, fora do escopo desta versão.
- ℹ️ `fn_carga_mensal_consolidada` devolve 0 linhas para a `service_role`: o guard de papel bypassa quando `auth.uid() IS NULL` e o de escopo não. Não afeta uso real (a tela chama com a sessão do usuário), mas torna a função indepurável por fora.
- ⚠️ **`historico_transferencias` ficou de fora** da correção de policy: a `FOR ALL` de lá tem a mesma folga. É log — escrever nele não move ninguém —, mas continua sendo pendência.
- ⚠️ Continuam abertas desde 23/08 a auditoria das **533 marcações sintéticas** de origem `terminal` e os **10 dias (13h30) com campo de origem `manual`** em 08/2026.

## [2.21.0] - 2026-08-27

### Added
- **Autorização do RH para justificativa coletiva de ponto (Ofício 249/2026).** Os 7 técnicos do Programa Porta a Porta iniciam a jornada de madrugada, são buscados em casa pelo motorista e vão direto ao atendimento — o ofício pede dispensa do registro de **entrada** e mantém, por escrito, a obrigatoriedade da **saída** na sede.
  - ⚠️ **O achado que mudou o desenho: metade disso já existia.** O "⚡ Validar em Massa" já aceitava vários servidores, o mês inteiro e uma justificativa única — o coordenador nunca precisou justificar dia a dia. Faltavam duas coisas, e são elas que viraram código.
  - **A autorização** (`autorizacoes_ponto_coletivo`, `20260827020000`): **nominal por servidor** — o ofício autoriza pessoas, não setores, então servidor novo no setor não herda nada. Guarda passos liberados, vigência, ofício e motivo, todos obrigatórios. Revoga-se, nunca se apaga. Conceder e revogar: **só RH Geral e Administrador Geral**, conferido dentro da função. Tela na aba **"Autorizações do RH"** em `/marcacoes`.
  - **O modo restrito** (`fn_atestar_passos_autorizados_bulk`): confere a autorização **dia a dia** — a vigência pode começar no meio do período selecionado — e declara só os passos liberados. No modal aparece o botão **🔒 Autorizado pelo RH**, visível apenas quando algum dos selecionados tem autorização vigente, já com a justificativa oficial preenchida. Quem não tem autorização é pulado e volta **nominalmente** na mensagem de resultado.
  - **A folha cita o ofício**: `REGISTRO DE ENTRADA DISPENSADO CONF. OFÍCIO 249/2026` na observação do dia, aplicado nas **quatro** cópias da geração por gerador com contagem de ocorrências.
  - ⚠️ **Isto não é marcação automática pelo sistema** (vedação 2 da Portaria 671/2021): é o coordenador declarando, com justificativa, exatamente como a validação manual já fazia — origem `ajuste_coordenador`, `sintetica = true`, rotulada como **manual** na folha. O que a migration acrescenta é a autorização prévia do RH como pré-condição, conferida no banco (a RPC é chamável direto — armadilha 12).
  - ⚠️ **A saída nunca é tocada**: nenhum caminho passa `'saida'`, `'completo'`, `'periodo_1'` ou `'periodo_2'`. `CHECK` no banco impede `saida` na lista de passos, a vigência é obrigatória com teto de 12 meses, e um trigger recusa autorizações sobrepostas para a mesma pessoa.
  - ⚠️ **Nenhuma função de presença foi alterada** (armadilha 1): `fn_confirmar_presenca_manual` já aceitava `p_tipo = 'entrada'`, `'intervalo_saida'` e `'intervalo_retorno'` isolados desde sempre, e nunca teve chamador — a função nova a envelopa uma vez por passo.
  - ⚠️ **Armadilha encontrada no caminho:** `p_tipo = 'intervalo_retorno'` **fabrica a entrada** (início + 5h) quando ela está nula. Se a autorização liberar retorno de intervalo **sem** liberar entrada, a função pula o dia em que a entrada ainda não existe — senão criaria pelas costas exatamente o horário que o desenho existe para não criar.
- **Excluir setor, exclusivo do Administrador Geral** (`fn_excluir_setor`, `20260827010000`). Setor cadastrado errado não tinha saída: a tela só oferecia Inativar. Medido: **225 dos 645 setores não têm vínculo nenhum**. A função **recusa listando os vínculos** em vez de deixar a FK agir — parte das FKs é `ON DELETE CASCADE`/`SET NULL` e apagaria ou anularia dado real em silêncio. A varredura é dinâmica sobre `pg_constraint`: `servidores`, `escala_mensal` e `profile_setores` nasceram fora do versionamento (armadilha 2), e uma lista escrita à mão nasceria incompleta.
- **Busca direta de servidor em Afastamentos.** Campo incremental por nome, matrícula ou CPF (ignorando acentos e pontuação) que preenche unidade e setor ao escolher — o RH gerencia **1.318 servidores ativos** e não tem como saber a lotação de cada um de cabeça. Carga paginada por `Range` (o PostgREST corta em 1000 em silêncio, armadilha 8) e em segundo plano, sem atrasar o formulário; resultados ordenados por relevância, porque "silva" casa com 351 pessoas.
- **C4 — mover batida real entre turnos do mesmo dia** (`fn_reclassificar_passo_entre_turnos`, `20260827060000`). Último item aberto do plano de 23/08: move batida **real** entre LINHAS de `escala_diaria` do mesmo servidor e do mesmo dia — a saída classificada no expediente quando pertencia ao plantão. Guards: mesmo servidor e mesmo dia, Sobreaviso recusado, competência aberta, escopo dentro da própria RPC, origem com batida real e destino vazio. Escreve no destino **antes** de limpar a origem, para que uma falha devolva a batida ao lugar; `marcacoes_ponto` nunca é tocada. Medido: **66 dias com alguma linha pela metade** em 08/2026, 55 deles com batida real para mover. 🚧 **Ainda sem action e sem tela.**

### Fixed
- 🚨 **Qualquer pessoa podia gravar presença em folha de ponto sem login.** Medido com a chave anon — que vai no bundle do navegador — e sem nenhuma sessão: `fn_confirmar_presenca`, `fn_confirmar_presenca_manual`, `fn_confirmar_presenca_manual_bulk`, `fn_atestar_jornada_bulk`, `fn_atestar_passos_autorizados_bulk` e `fn_registrar_ponto` respondiam **HTTP 200**.
  - ⚠️ **A causa é o padrão usado em todo o projeto.** `CREATE FUNCTION` já concede `EXECUTE` a **PUBLIC**; `GRANT EXECUTE ... TO authenticated, service_role` é inofensivo e **inútil como restrição** — quem não está na lista continua entrando por PUBLIC. Das 394 funções do schema, **369 eram alcançáveis por anon**, e as 25 fechadas eram exatamente as que escreveram `REVOKE ... FROM PUBLIC`.
  - ⚠️ **E a primeira correção aplicou "com sucesso" sem mudar nada**: `REVOKE` de quem não é dono da função **não falha — emite `WARNING` e segue**. Só se descobriu medindo por fora. Desde então **toda migration de privilégio confere o próprio resultado e aborta**, com banco, usuário e dono de cada função pendente na mensagem de erro.
  - `20260827040000` fechou o núcleo de presença (369 → **353** funções visíveis a anon); `20260827050000` fechou escala, justificativas, avisos de ponto, biometria e logs (353 → **324**), verificando **os dois sentidos** — aborta se anon continuar entrando **e** se alguma função que a tela usa perder `authenticated`, porque derrubar a grade do coordenador seria trocar um problema por outro pior, e igualmente em silêncio.
  - **Estado final:** das funções alcançáveis por anon, as que escrevem sem conferir papel caíram de **35 para 4** — e as 4 são o fluxo público do link mágico de sobreaviso, que **exigem `magic_token`**. Registrado como **armadilhas 24 e 25** no `CLAUDE.md`.
- 🚨 **O terminal clássico continuava funcionando com a chave desligada.** `terminal_classico_habilitado = false` era lido só na sidebar e na tela de login, para esconder o botão: a rota `/presenca` continuava servida e `fn_registrar_ponto` continuava alcançável. **Medido: a chave foi desligada em 21/08 às 00:13 e, depois disso, entraram 97 batidas reais de 18 servidores em 2 unidades**, com 6 coordenadores supervisionando — e não há nenhum terminal local cadastrado, ou seja, a chave foi desligada antes de o substituto existir.
  - Trava em três camadas (`20260827000000`): o banco recusa antes de qualquer escrita e antes de conferir o PIN; o middleware não serve mais a rota (confirmado em produção: `307 → /login?terminal=desativado`); a tela mostra "Terminal Desativado", sem formulário. O Terminal Local não morre junto — `fn_registrar_ponto_terminal_local` publica o GUC `sisescala.canal_ponto` antes de delegar.
  - ⚠️ **Não é restrição de horário** (vedação 1 da Portaria 671/2021): nada aqui olha a hora da batida. Onde o canal está ligado, a regra de nunca recusar por horário continua valendo byte a byte.
- **`fn_atestar_jornada_bulk` anunciava "0 registro(s)" mesmo tendo gravado.** `fn_confirmar_presenca_manual_bulk` devolve `processed_count` (assim desde `20260804040000`) e a função lia `total_processed`, que nunca existiu — nas duas versões dela, desde 08/08/2026. Nada era gravado errado: a Validação em Massa funcionava e dizia que não tinha feito nada. É a armadilha 22 na forma pior — **relatar zero quando mudou**.
- **Os filtros da tela de usuários não filtravam.** Casavam contra o **escopo de permissão** da conta, não contra a **lotação** exibida no card, e com três coringas. Filtrar o setor **DMAC** devolvia **71 de 95 contas** (38 entravam só por Acesso Total, 26 só por terem a unidade), contra 7 vinculadas ao setor e 6 lotadas nele. Agora casam por **lotação OU vínculo explícito**: SND vai de 59 para 1, RECURSOS HUMANOS de 59 para 5, unidade CEI de 11 para 2. O dropdown de setores — 644 setores com **79 nomes repetidos**, "RECEPÇÃO" aparece 33 vezes — passou a ser agrupado por unidade quando nenhuma unidade está escolhida.
- **Setor inativo aparecia nos dropdowns de escolha** de `/servidores/pendencias`: a consulta nunca filtrou por `ativo` (17 dos 645 estão inativos). Promoção de pendência e destino de transferência passam a oferecer só setor ativo; a lista completa continua chegando para **exibir** lotação de origem já desativada, e o destino que uma solicitação antiga já trazia continua selecionável. Filtros de relatório não foram tocados — ali esconder inativo quebraria leitura de dado histórico.
- **O verso da folha listava sábados e domingos como justificativa.** O relatório incluía todo dia com `observacao` preenchida — e a geração escreve `SÁBADO`/`DOMINGO`/`FOLGA` sozinha em todo dia sem escala. Medido nas **482 folhas de 08/2026: 11.505 linhas, das quais 6.216 eram fim de semana** e 451 feriado sem trabalho, todas assinadas como **"Gestão / Coordenação"** — o documento afirmava que alguém justificou o que ninguém justificou. Ficam **4.838 linhas**, e **85 folhas** passam a ter o verso vazio, exibindo "Nenhuma ocorrência extraordinária ou ajuste manual registrado no período".
  - ⚠️ **A parte humana é preservada quando vem colada no rótulo** (`AFASTAMENTO PARCIAL: DECLARAÇÃO DE COMPARECIMENTO (M) | SÁBADO`): a função separa por `|` e descarta só os rótulos automáticos.
  - ⚠️ **Trabalho EM feriado passa a aparecer**, com o horário — aí é ocorrência de verdade. Feriado sem trabalho é calendário, e a frente da folha já mostra o dia.
  - Fonte única em `src/utils/folha/ocorrencias.ts`: havia **duas cópias** do relatório, no editor da folha e na impressão em lote, com regras diferentes entre si. Nada altera dado — o verso é derivado de `folha_ponto.registros` na renderização.

### Changed
- **RH Geral e RH da Unidade editam a grade fora do prazo de planejamento, como Diretor.** O bloqueio nunca foi escala fechada: `canEditScale` liberava só `super_admin` e `admin`, e o RH batia no dia limite. Fonte única nova em **`podeEditarForaDoPrazo`** (`src/utils/governance.ts`) — o teste `role !== 'admin' && role !== 'super_admin'` estava repetido em **6 lugares** do `ScaleGrid`, e era essa a armadilha de acrescentar papel. `rh_unidade` continua escopado pela RLS: liberar o prazo não amplia o que ele enxerga, só quando pode mexer no que já é dele.
- O texto da configuração do terminal clássico deixou de prometer o comportamento defeituoso ("o botão fica escondido, mas o recurso não é removido").

### Known
- ⚠️ **8 dos 18 servidores que usavam o terminal clássico não têm nenhuma batida no relógio em agosto** (CARMEN LUCIA, DENISVAL, FAGNER, FRANCISCO GLEUSSON, GUILHERME, HELEN MAIKEL, LAUREN, MARCELO MEDEIROS). Com a trava aplicada e a chave desligada, ficam sem meio de registrar ponto. A chave é reversível num clique em Configurações → Regras.
- ⚠️ **3 dos 7 nomes do Ofício 249/2026 não casam com o cadastro**: "Gessica Francielle Almeida Barbosa" está como **GÉSSICA FRANCIELE ALMEIDA BARBOSA** (mat. 68246); **Luzinete Martins da Silva** e **Nídia Evilyn Souza Costa** não aparecem lotadas no Porta a Porta. A autorização é nominal — sem conferir com o RH, essas pessoas são ignoradas pelo modo 🔒.
- ⚠️ **O fluxo da autorização ainda não rodou com dados reais.** A primeira execução merece ser um mês só, com conferência à mão junto ao RH. O Porta a Porta ainda não entrou no fluxo: 10 escalas de 08/2026, **todas em Rascunho**, zero dias lançados e zero marcações.
- ℹ️ **Duas contagens diferentes de exposição a anon:** `pg_proc` diz 818 de 890, mas inclui as centenas de funções do PostGIS e as de **trigger**, que o PostgREST não expõe como RPC. O que é alcançável de fora são as 324 do OpenAPI.
- ℹ️ **As tabelas nunca estiveram expostas**: a RLS já devolvia 0 linhas para anon em todas. A única legível é `configuracoes_globais`, com allowlist de **7 chaves inócuas** (logo, timezone, flags) — `email_smtp_senha` e `whatsapp_astracall_key` não aparecem.
- ⚠️ Continuam abertas desde 23/08 a **auditoria das 533 marcações sintéticas** de origem `terminal` (`fn_salvar_saida_bloco` fabrica horários de transição) e os **10 dias (13h30) com campo de origem `manual`** em 08/2026, que só o coordenador desfaz.
