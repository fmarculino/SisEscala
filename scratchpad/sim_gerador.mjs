...
let setoresComResultado = 0, celulas = 0
...
for (const setorId of setores) {
  ...
  const escalaMensal = doSetor.map(e => ({ ...e, servidores: {} }))

  const r = await G.gerarEscalaInteligente(fake(setorId), {
    unidadeId: doSetor[0].unidade_id, setorId, mes: ALVO_MES, ano: ALVO_ANO,
    escalaMensal, turnos,
    options: {
      respectContinuity: true, respectEvents: true, respectPreferences: true,
      categorias: CATS, mesesHistorico: 3
    },
    quantidadeMeses: 1
  })
  ...
  const g = r.meses[0]
  if (!g) continue
  // Setor sem competencia anterior nenhuma nao entra na conta: o motor nao errou, ele nao
  // tinha o que aprender. Incluir esses 85 setores no denominador mediria o parque, nao o motor.
  if (r.mesesDeOrigemEncontrados === 0) continue
  if (g.total > 0) setoresComResultado++
  celulas += g.total
  ...
  for (const e of doSetor) {
    ...
    for (const c of CATS) {
      for (const [d, t] of Object.entries(prev[c] || {})) {
        acc[c].previu++
        if (real[`${c}|${d}`] === t) acc[c].acertou++
      }
    }
  }
}

console.log(`setores processados: ${setores.length} | com sugestao: ${setoresComResultado} | celulas sugeridas: ${celulas}\n`)
console.log('BACKTEST DO MOTOR NOVO — prever 08/2026 (3 meses de historico, todas as categorias)')
for (const c of CATS) {
  const a = acc[c]
  console.log(`  ${c.padEnd(11)} real=${String(a.real).padStart(5)} sugeriu=${String(a.previu).padStart(5)} acertou=${String(a.acertou).padStart(5)}` +
    `  cobertura=${a.real ? (a.acertou / a.real * 100).toFixed(1) : '0.0'}%  precisao=${a.previu ? (a.acertou / a.previu * 100).toFixed(1) : '0.0'}%`)
}
