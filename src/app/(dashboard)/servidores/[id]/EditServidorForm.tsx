...
export function EditServidorForm({ id, servidor, unidades, setores, cargos, isSuperAdmin = false, escalas = [] }: EditServidorFormProps) {
    ...
    <div className="rounded-2xl bg-white p-8 shadow-xl dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
      ...
      <form action={handleSubmit} className="space-y-6">
        ...
        <div className={formTab === 'principal' ? 'space-y-6 animate-in fade-in' : 'hidden'}>
          ...
          {/* 1. Nome Completo (Largura Total / Destaque) */}
          <div className="sm:col-span-6">
            <label htmlFor="nome" className="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-1">
              Nome Completo
            </label>
            <input
              type="text"
              ...
              required
              className="block w-full rounded-xl border border-zinc-300 bg-zinc-50 px-4 py-2.5 text-zinc-900 font-semibold dark:bg-zinc-800 dark:border-zinc-700 dark:text-white sm:text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm transition-all"
            />
          </div>

          {/* 2. Matrícula (3 colunas) */}
          <div className="sm:col-span-3">
            <label htmlFor="matricula" className="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-1">
              Matrícula
            </label>
            <input
              type="text"
              ...
              className={`block w-full rounded-xl border px-4 py-2.5 sm:text-sm font-mono font-bold shadow-sm transition-all focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                isTemporary
                  ? 'bg-amber-50 border-amber-300 text-amber-900 dark:bg-amber-900/10 dark:border-amber-900/30 dark:text-amber-300'
                  : 'bg-zinc-50 border-zinc-300 text-zinc-900 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white'
              }`}
            />
            {isTemporary && (
          ...
          {/* 4. Informação das Escalas Atuais do Servidor */}
          <div className="sm:col-span-6 bg-gradient-to-br from-blue-50/60 via-slate-50 to-indigo-50/40 dark:from-zinc-900/80 dark:via-zinc-900 dark:to-blue-950/20 border border-blue-100 dark:border-zinc-800 p-5 rounded-2xl shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-blue-100/60 dark:border-zinc-800/80 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-extrabold text-zinc-900 dark:text-white flex items-center gap-1.5">
                    Escalas Atuais / Ativas do Servidor
                  </h3>
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
                    Grade(s) de escala em que este servidor está incluído no momento
                  </p>
                </div>
              </div>
                ...
                {escalasAtivas.slice(0, 4).map((esc: any) => {
                  ...
                  return (
                    <Link
                      ...
                      target="_blank"
                      className="group relative p-3.5 bg-white dark:bg-zinc-850 border border-zinc-200/80 dark:border-zinc-750 rounded-xl hover:border-blue-500 dark:hover:border-blue-400 hover:shadow-md hover:-translate-y-0.5 transition-all flex items-center justify-between gap-3 cursor-pointer"
                      title="Clique para ir direto para a escala deste servidor"
                    >
                      <div className="space-y-1 min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-sm text-zinc-900 dark:text-white capitalize group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">
                            {mesNome}
                          </span>
                        ...
                        <p className="text-xs text-zinc-700 dark:text-zinc-200 font-semibold truncate flex items-center gap-1">
                          {esc.setor_nome || 'Setor não informado'}
                        </p>
                        <p className="text-[11px] text-zinc-400 dark:text-zinc-500 truncate flex items-center gap-1">
                          <MapPin className="h-3 w-3 shrink-0 text-zinc-400" />
                          {esc.unidade_nome || 'Unidade não informada'}
                        </p>
                      </div>

                      <div className="shrink-0 flex items-center gap-1 px-2.5 py-1.5 bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 group-hover:bg-blue-600 group-hover:text-white dark:group-hover:bg-blue-500 dark:group-hover:text-white text-xs font-bold rounded-lg transition-all shadow-2xs">
                        <span>Acessar</span>
