 ...
 */
export async function enviarWhatsAppInterno({ phone, message, unidadeId, overrideConfigs }: WhatsAppSendParams): Promise<WhatsAppResult> {
 ...
 */
export async function enviarEmailInterno({ to, subject, html, text, unidadeId, overrideConfigs }: EmailSendParams): Promise<EmailResult> {
  const dbConfigs = await getCommunicationConfigs()
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      tls: {
        // Valida o certificado do servidor SMTP. Antes era `false` fixo, com o comentario
        // "flexibilidade para ambientes corporativos com certificados autoassinados" — mas o
        // host configurado em producao e' `smtp.gmail.com` (conferido em 30/08/2026), que tem
        // certificado publico valido. Ou seja: a flexibilidade nao estava sendo usada por
        // ninguem, e desligava a unica defesa contra alguem se passar pelo servidor de SMTP e
        // colher usuario/senha no AUTH.
        //
        // A escotilha continua existindo, mas agora e' DECISAO EXPLICITA por instalacao, e nunca
        // o padrao: `email_smtp_tls_inseguro = "true"` em configuracoes_globais (ou na config da
        // unidade). Se o e-mail parar de funcionar depois desta mudanca, o certificado do host e'
        // que e' invalido — confira o host antes de ligar a chave.
        rejectUnauthorized: String(configs['email_smtp_tls_inseguro'] ?? '') !== 'true'
      }
    })
